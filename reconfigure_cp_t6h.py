...
g_log_folder = "/mnt/test_log/{}/RM_logs".format(g_project)
g_sf_config_folder = "/WIN/{}/sfconfig".format(g_project.upper())
g_DHCP_folder = "/var/lib/dhcpd"
g_project_folder = "/project"
g_tftp_folder = "/tftpboot"
...
if build_side == "QMF":
    ...
    g_sf_config_folder = "/WIN/{}/sfconfig".format(g_project.upper())
if build_side == "QCG":
    g_log_folder = "/mnt/testlog/{}/RM_logs".format(g_project) #QCG
    g_sf_config_folder = "/mnt/win/{}/sfconfig".format(g_project.upper()) #QCG
    g_DHCP_folder = "/var/lib/dhcp"
    g_tftp_ip = "10.30.63.253"
    g_project_folder = "/home/project"
    g_tftp_folder = "/home/tftpboot"
if build_side == "QCI":
    g_log_folder = "/testlog/{}/RM_logs".format(g_project)
    g_sf_config_folder = "/mnt/smbfs/Wayne_Xu/{}/CP_config/sfconfig".format(g_project.upper())
    g_DHCP_folder = "/mnt/smbfs/__DHCP"
    g_tftp_ip = "192.168.0.253"
    g_tftp_user = "test"
    g_tftp_passwd = "qcitest"

g_PXE_grub_folder = "{}/pxelinux.cfg".format(g_tftp_folder)

g_reboot_flag = False
g_cp_FPGA_firmware="CelestialPeak_SysInt_{}_jic.rpd"
g_cp_FPGA_firmware_loc="{}/firmware/{}/cp/{}".format(g_project_folder,g_project,g_cp_FPGA_firmware)
g_overlake_image_path = "{}/firmware/Celestial_Peak/dropcp-{}FW-stos{}/rMedia"
...
g_overlake_md5sum = {"overlake-2008.6.23101601-prod-stos.img":"e5075189b5e8cfdb0017e21a1d6b3d86",
                     "overlake-2008.6.23101601-prod.img":"784d222f1f8b233543be00c5c1c90bd3",
...
# Notice     : NA
#*****************************************************************************
def sendlog(message = "", color = 0, file = "{}/monitor.log".format(g_log_folder)):
    if color == 1 :
        ESC_color = ESC_GREEN
    elif color == 2 :
        ESC_color = ESC_RED
...
def get_ip(mac_addr):
    ...
    if build_side == "QTMC" or build_side == "QMF" or build_side == "QCG":
        search_ip_cmd = "grep -B8 -A1 {0} {1}/dhcpd.leases | grep lease".format(mac_trans,g_DHCP_folder)
        ...
        search_ip_cmd = "grep -B2 -A1 {0} {1}/dhcp2.txt | grep \"^192\" | awk -F \":\" '{{print $1}}'".format(mac_trans,g_DHCP_folder)
        ip = getresult(search_ip_cmd)
        ip = ip.split("\n")
...
def update_cp_fpga_fw(MBSNdict, log_path):
    cmd = "find {}/firmware/{0}/ -name *{1}*.rpd".format(g_project_folder,g_project.lower(),MBSNdict["CP_GOLDEN_IMAGE"].split(',')[0])
    res = getresult(cmd)
    if res != "":
...
def get_mbsn_info_from_sf_folder(mbsn_info_list):
    ...
    for mbsn in mbsn_list:
        ...
        if MBSNdict["cp_ip"] == "" or MBSNdict["cp_ip"] == 1:
            continue
...
def chk_soc_fw(MBSNdict):
    ...
    if MBSNdict["CP_SOC_FIP_FW"] != get_soc_fw_ver:
        ...
        if MBSNdict["CP_CERBERUS_FW"] != get_soc_cerberus_fw_ver:
            sendlog("SoC cerberus firmware version check FAIL ! exp:{0}, get:{1}".format(MBSNdict["CP_CERBERUS_FW"], get_soc_cerberus_fw_ver), RED, log_path)
...
def update_soc_fw(MBSNdict):
    ...
    log_path = "{0}/{1}".format(sn_log_folder, log_name)
    image_path = "{}/{}".format(g_tftp_folder,g_os_path)
    try:
        os.makedirs(sn_log_folder)
        sendlog("make log folder ({0}) success".format(sn_log_folder), GREEN, log_path)
    except OSError as e:
    ...
    if build_side == 'QCI':
        ...
        if image_path not in result:
            sendlog("MBSN:{0} no overlake image,{1} on PXE server~~~~~~~~~".format(MBSNdict["mbsn"],image_path), 0, log_path)
            return 1
    else:
    ...
    if build_side == 'QCI':
        ...
        os.system("echo 'label T6H rsa image' > {}".format(image_mac_file))
    ...
    #20250520 WayneXu 0002.17.240911-prd use the same image name compare to previous version
    if MBSNdict["CP_SOC_FIP_FW"] in replace_image_name and MBSNdict["CP_SOC_NITRO_FW"] in replace_image_name:
        upgrade_image = "overlake-{}-prod-stos.img"
    cmd = "find {}/firmware/Celestial_Peak/ -name {}".format(g_tftp_folder,upgrade_image.format(MBSNdict["CP_SOC_OS_FW"]))
    ...
    if build_side == 'QCI':
        ...
        #image_path = g_overlake_image_path.format(g_tftp_folder,MBSNdict["CP_SOC_FIP_FW"][3:7],MBSNdict["CP_SOC_OS_FW"])
        image_path = g_overlake_image_path
        #20250421 WayneXu 0002.17.240911-prd use the same image name compare to previous version
        if MBSNdict["CP_SOC_FIP_FW"] in replace_image_name and MBSNdict["CP_SOC_NITRO_FW"] in replace_image_name:
            image_path = image_path.replace(replace_image_name[MBSNdict["CP_SOC_FIP_FW"]][0],replace_image_name[MBSNdict["CP_SOC_FIP_FW"]][1])
        cmd = "sshpass -p '{0}' scp -o 'StrictHostKeyChecking no' {1}@{2}:{3}/{4} /tmp/".format(g_tftp_passwd, g_tftp_user, g_tftp_ip, image_path,upgrade_image.format(MBSNdict["CP_SOC_OS_FW"]))        
        result = getresult(cmd).split('\n')
        sendlog(result, 0, log_path)
        ...
        if MBSNdict["CP_SOC_FIP_FW"] in replace_image_name and MBSNdict["CP_SOC_NITRO_FW"] in replace_image_name:
            image_path = image_path.replace(replace_image_name[MBSNdict["CP_SOC_FIP_FW"]][0],replace_image_name[MBSNdict["CP_SOC_FIP_FW"]][1])
        cmd = "sshpass -p overlake scp -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null {0}/{1} stuser@{2}:/tmp/".format(image_path,upgrade_image.format(MBSNdict["CP_SOC_OS_FW"]), MBSNdict["cp_ip"])
    ...
    g_cp_Cerberus_fw_loc="{}/firmware/{}/cp/{}".format(g_project_folder,g_project,g_cp_Cerberus_fw)
    if build_side == "QCI":
        os.system("mkdir -p {}/firmware/{}/cp/".format(g_project_folder,g_project))
...
def power_cycle_uut(MBSNdict):
    sn_log_folder = "{0}/{1}".format(g_log_folder, MBSNdict["mbsn"])
    log_name = "{0}_reboot_log.txt".format(MBSNdict["mbsn"])
    log_path = "{0}/{1}".format(sn_log_folder, log_name)

    ret = getresult("ipmitool -H {0} -U {1} -P {2} -I lanplus power cycle".format(MBSNdict["bmc_ip"], g_bmc_username, g_bmc_password))
    sendlog(ret, 0, log_path)
    if "Chassis Power Control: Cycle" in ret:
        sendlog("Power cycle success", 0, log_path)
    else:
        sendlog("Can not execute power cycle through ipmitool, sleep a while and try again!", 0, log_path)
        time.sleep(10)
        ret = getresult("ipmitool -H {0} -U {1} -P {2} -I lanplus power cycle".format(MBSNdict["bmc_ip"], g_bmc_username, g_bmc_password))
        sendlog(ret, 0, log_path)
        if "Chassis Power Control: Cycle" in ret:
    ...
    time.sleep(100) #wait 100 secs for system reboot
    return 0
