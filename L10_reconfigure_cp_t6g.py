...
g_sf_config_folder = "/WIN/{}/sfconfig/".format(g_project.upper()) #QMF2
...
if build_side == "QTMC":
	g_log_folder = "/mnt/test_log/{}/RM_logs".format(g_project) #QTMC
	g_sf_config_folder = "/win/monitor/Manual/" #QTMC
	g_cp_project_fw_path="/project/firmware/{}/cp/".format(g_project)
	g_tftp_ip = "172.22.0.1"
	g_tftp_user = "test"
	g_tftp_passwd = "qcitest"
elif build_side == "QMF":
	g_log_folder = "/RACKLOG/{}/RM_logs".format(g_project) #QMF
	g_sf_config_folder = "/WIN/{}/sfconfig/".format(g_project.upper()) #QMF2
	g_cp_project_fw_path="/project/firmware/{}/cp/".format(g_project)
	...
	g_log_folder = "/mnt/test_log/{}/RM_logs".format(g_project) #QTMC
	g_sf_config_folder = "/cp_monitor/monitor/" #QCI
	g_cp_project_fw_path="/mnt/smbfs/firmware/{}/cp/".format(g_project)
	g_tftp_ip = "192.168.0.253"
	g_tftp_user = "test"
	g_tftp_passwd = "qcitest"
...
def sendlog(message = "", color = 0, file = "/RACKLOG/{}/RM_logs/monitor.log".format(g_project)):
	if color == 1 :
		ESC_color = ESC_GREEN
	elif color == 2 :
		ESC_color = ESC_RED
	...
	cmd = ("echo \"{0} ====>> {1}\" >>{2}".format(time.strftime("%m-%d-%y-%H:%M:%S"), message, file)) # 20240508 David fix for new fpga fw have ' word cause cannot save log
...
def show_info():
	...
	sendlog ("\033[1;36;40m*\033[1;33;40m MSFT_Monitor_Reconfig_CP  -\033[0m  V0.15 2025/03/14 Release By Dylan \033[1;36;40m*\033[0m")
	sendlog ("\033[1;36;40m*\033[1;33;40m MSFT_Monitor_Reconfig_CP  -\033[0m  V0.16 2025/03/19 Release By Dylan \033[1;36;40m*\033[0m")
	sendlog ("\033[1;36;40m***********************************************************************\033[0m")
	sendlog ("\n")
...
def get_mbsn_info_from_sf_folder(mbsn_info_list):
	...
	for mbsn in mbsn_list:
		...
		MBSNdict = {
			...
			"is_reconfig": True,
			"CP_CERBERUS_FW": "",
			"CP_FPGA_FW": "", # Dylan, 2024/10/29, add to sync C2789 chk_golden_image_log_file().
			"CP_GOLDEN_IMAGE": "",
			"CP_FACTORY_IMAGE": "",
			"CP_SOC_OS_FW": "",
			"CP_SOC_FIP_FW": "",
		...
		for file_name in info:
			if ".txt" in file_name and mbsn in file_name:
				...
				for line in f:
					...
					if "CP_FPGA_FW" in line: # Dylan, 2024/10/29, add to sync C2789 chk_golden_image_log_file().
						print ("line = {0}".format(line.lower()))
					...
					if "CP_SOC_FIP_PFMID" in line:
						print ("line = {0}".format(line.lower()))
						cp_soc_fip_pfmid = line.lower().split('=')[1].strip()
					if "CP_SOC_NITRO_PFMID" in line:
						print ("line = {0}".format(line.lower()))
						cp_soc_nitro_pfmid = line.lower().split('=')[1].strip()
				f.close()
		#BSNdict["mbsn"] = mbsn
		...
		MBSNdict["CP_FPGA_FW"] = cp_fpga_fw # Dylan, 2024/10/29, add to sync C2789 chk_golden_image_log_file().
		...
		MBSNdict["CP_SOC_FIP_PFMID"] = cp_soc_fip_pfmid
		MBSNdict["CP_SOC_NITRO_PFMID"] = cp_soc_nitro_pfmid
...
def is_CP_Golden_mode(MBSNdict):
	...
	if "Found no catapult devices" in result:
		sendlog("No catapult devices !!! ".format(sn_log_folder), RED)
		return 6
	...
	if not os.path.isfile(record_log_path):
		...
		for i in range(5):
			...
			if golden_image_chk_result == 0:
				chk_count += 1
				sendlog("{0}_do_record_CP_golden_image_version.log content check PASS!".format(MBSNdict["mbsn"]))
				break
			else:
				sendlog("{0}_do_record_CP_golden_image_version.log content check FAIL!".format(MBSNdict["mbsn"]))
				sendlog("Re-write content in {0}_do_record_CP_golden_image_version.log. Retry:{1}/5".format(MBSNdict["mbsn"], i+1))
				result = write_golden_image_log_file(MBSNdict)
				if result == 2: #No catapult devices
					return 6
		...
		for line in file:
			for i in range(len(chk_list)):
				if chk_list[i] == "Read register 101" and chk_list[i] in line:
					...
					#print("get_read_register_101, CP_GOLDEN_IMAGE => exp = {}, get = {}".format(get_read_register_101, MBSNdict["CP_GOLDEN_IMAGE"].split(",")[1])) # Dylan, 2024/10/30, add for debug.
				...
				if chk_list[i] == "Read register 65" and chk_list[i] in line:
					...
					#print("get_read_register_65, CP_FPGA_FW => exp = {}, get = {}".format(get_read_register_65, MBSNdict["CP_FPGA_FW"].split(",")[1])) # Dylan, 2024/10/30, add for debug.
					...
					#print("get_role_id_1, CP_GOLDEN_IMAGE => exp = {}, get = {}".format(get_role_id_1, MBSNdict["CP_GOLDEN_IMAGE"].split(",")[1])) # Dylan, 2024/10/30, add for debug.
					...
					#print("exp = {}, get = {}".format(get_role_ver_1, get_fpga_fw)) # Dylan, 2024/10/30, add for debug.
				...
				if chk_list[i] == "[FPGA-CONFIG-EX ]" and chk_list[i] in line:
					...
					#print("get_role_id_2, CP_GOLDEN_IMAGE => exp = {}, get = {}".format(get_role_id_2, MBSNdict["CP_GOLDEN_IMAGE"].split(",")[1])) # Dylan, 2024/10/30, add for debug.
					if get_role_id_2 == (MBSNdict["CP_GOLDEN_IMAGE"].split(",")[1]):
						get_info += 1
					#print("get_golden_version, CP_GOLDEN_IMAGE => exp = {}, get = {}".format(get_golden_version, MBSNdict["CP_GOLDEN_IMAGE"].split(",")[0])) # Dylan, 2024/10/30, add for debug.
					if get_golden_version == (MBSNdict["CP_GOLDEN_IMAGE"].split(",")[0]):
						get_info += 1
					break
				if chk_list[i] == "golen_image_role_id" and chk_list[i] in line:
					get_role_id_3 = line.split(" ")[9].strip().strip("(").strip(")")
					#print("get_role_id_3, CP_GOLDEN_IMAGE => exp = {}, get = {}".format(get_role_id_3, MBSNdict["CP_GOLDEN_IMAGE"].split(",")[1])) # Dylan, 2024/10/30, add for debug.
					if get_role_id_3 == (MBSNdict["CP_GOLDEN_IMAGE"].split(",")[1]):
						get_info += 1
					break
				if chk_list[i] == "golden_image_version" and chk_list[i] in line:
					get_role_ver_2 = line.split(" ")[9].strip().strip("(").strip(")")
					#print("get_role_ver_2, CP_FPGA_FW => exp = {}, get = {}".format(get_role_ver_2, MBSNdict["CP_FPGA_FW"].split(",")[1])) # Dylan, 2024/10/30, add for debug.
					if get_role_ver_2 == MBSNdict["CP_FPGA_FW"].split(",")[1]:
						get_info += 1
					break				 
				if (chk_list[i] in line) or (chk_list[i] == "Read register 59" and chk_list[i] in line) or (chk_list[i] == "golden_image_build_version" and chk_list[i] in line):
					#print("exp = {}, get = {}".format(chk_list[i], line)) # Dylan, 2024/10/30, add for debug.
					get_info += 1
					break
	#print("exp_info = {}, get_info = {}".format(exp_info, get_info)) # Dylan, 2024/10/30, add for debug.
	if get_info == exp_info:
		sendlog("{} golden image log file content check PASS!".format(MBSNdict["mbsn"]))
		return 0
	else:
		sendlog("{} golden image log file content check FAIL!".format(MBSNdict["mbsn"]))
		return 1
...
def write_golden_image_log_file(MBSNdict):
	...
	for i in range(len(info)):
		if "Chip ID" in info[i]:
			line_next = info[i+1]
			line_next = re.sub(r'\s+', ' ', line_next)  # Dylan, 2024/10/30, add this line for different size of space.
			CP_chip_id = line_next.split(" ")[1].strip()
...
#*****************************************************************************
# Function   : uut_send_status_to_sf
# Description: Send status to SFC
# Inputs     : None
# Outputs    : Pass : return 0 ; Fail : return 1
# Notice     : 
#*****************************************************************************	
def uut_send_status_to_sf(send_status, status_msg="", test_sn=""):
	monitor_url_status="/SFC/NetApp/Status" #/mnt/monitor/NetApp/Status
	#ip = self.gs_getresult("ifconfig -a | grep -m1 inet | awk '{{print$2}}' ")
	if test_sn == "":
		print("Can not get SN")
		return 1 
	send_file = '/tmp/{}.txt'.format(test_sn)
	os.system('rm {}'.format(send_file))
	start_time = time.strftime("%Y%m%d%H%M%S")
	station = "FAT"
	# global start_time
	with open(send_file, mode="at") as f:
		f.write("MBSN={}\n".format(test_sn))
		f.write("IP=\n")
		# Station
		f.write("Station={}\n".format(station))
		f.write("STARTT={}\n".format(start_time))
		# status
		if send_status.upper() == 'PASS':
			f.write("status={} finish successfully\n".format(station.upper()))
		elif send_status.upper() == 'FAIL':
			f.write("status=retest=Y;{} test FAIL, {}\n".format(station.upper(), status_msg))
		elif send_status.upper() == 'ERROR':
			f.write("status={} test error, {}\n".format(station.upper(), status_msg))
		elif send_status.upper() == 'START':
			f.write("status={} test start".format(station.upper()))
		else:
			f.write("status={}".format(station.upper()))

	os.system('busybox unix2dos {}'.format(send_file))
	shutil.copy(send_file, monitor_url_status)
	time.sleep(1)
	os.remove(send_file)
	return 0
#*****************************************************************************
...
def chk_soc_fw(MBSNdict):
	...
	if MBSNdict["CP_SOC_FIP_FW"] != get_soc_fw_ver:
		...
		if MBSNdict["CP_CERBERUS_FW"] != get_soc_cerberus_fw_ver:
			...
			#L10 diag will update cerberus fw. Skip to update cerberus fw if the fw is not match. Jenny 20240305
			#return 2
		else:
...
def update_soc_fw(MBSNdict):
	...
	log_path = "{0}/{1}".format(sn_log_folder, log_name)
	g_os_path = "firmware/{}/stos/Image_rsa_{}.img".format(g_project,MBSNdict["CP_SOC_FIP_FW"][3:7])
	g_cmd_emmcflash = "emmcflash -t {0} -a {1} -b {1} -s".format(g_tftp_ip, g_os_path)
	image_path = "/tftpboot/{}".format(g_os_path)
	g_cp_Cerberus_fw="cerberus_v{0}.bin".format(MBSNdict["CP_CERBERUS_FW"])
	g_cp_Cerberus_fw_loc="/project/firmware/{0}/cp/{1}".format(g_project.lower(),g_cp_Cerberus_fw)
	fip_pfm_file  = "A2040.FIP.PFM.{}.bin".format(int(MBSNdict["CP_SOC_FIP_PFMID"],16))
	fip_fw_file = "fip_padded.bin"
	nitro_pfm_file = "A2040.NITRO.PFM.{}.bin".format(int(MBSNdict["CP_SOC_NITRO_PFMID"],16))
	nitro_fw_file = "nitro.img"
	g_cmd_update_sop = ["cerberus_utility pfmupdate 0 /tmp/usb/{} 0".format(fip_pfm_file),
						"cerberus_utility socfwupdate 0 /tmp/usb/fip_padded.bin 1",
	...
	if build_side == 'QCI':
		...
		print("[debug]image_path:{}".format(image_path))
	...
	if build_side == 'QCI':
		...
		if os.path.isfile(image_mac_file):
			os.remove(image_mac_file)
		os.system("echo 'label {} rsa image' > {}".format(g_project, image_mac_file))
		os.system("echo 'kernel {}' >> {}".format(g_os_path, image_mac_file))
	...
	if res != "":
		...
		sendlog("[SN:{0}] Cannot find CP SOC image path".format(MBSNdict["mbsn"]))
		return 1

	if build_side == 'QCI':
		print ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Get CP overlake image on TFTP".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"]))
		cmd = "sshpass -p '{0}' scp -o 'StrictHostKeyChecking no' {1}@{2}:{3}/{4} /tmp/".format(g_tftp_passwd, g_tftp_user, g_tftp_ip, g_overlake_image_path,upgrade_image.format(MBSNdict["CP_SOC_OS_FW"]))
		print("[debug]cmd:{}".format(cmd))
		result = getresult(cmd).split('\n')
		sendlog(result, 0, log_path)
		...
		#Copy image from TFTP to CP SoC
		print ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Copy image from TFTP to CP SoC".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"]))
		cmd = "sshpass -p overlake scp -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null {0}/{1} stuser@{2}:/tmp/".format(g_overlake_image_path,upgrade_image.format(MBSNdict["CP_SOC_OS_FW"]), MBSNdict["cp_ip"])
		print("[debug]cmd:{}".format(cmd))
		result = getresult(cmd).split('\n')
		sendlog(result, 0, log_path)
	...
	#Check FIP/NITRO/Cerberus
	get_fip_pfmid = ""
	cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{} \"echo overlake | sudo -S /usr/bin/{}\"".format(MBSNdict["cp_ip"],  g_cmd_cerberus_pfm_chk.format(0))
	print("[debug]g_cmd_update_sop cmd:{}".format(cmd))
	result = getresult(cmd)
	sendlog(result, 0, log_path)
	if result == "":
		...
		for line in result.splitlines():
			if "Cerberus PFM ID:" in line:
				...
				break
	print("[debug]get_fip_pfmid:{}".format(get_fip_pfmid))
	print("[debug]CP_SOC_FIP_PFMID:{}".format(MBSNdict["CP_SOC_FIP_PFMID"]))
	if get_fip_pfmid != MBSNdict["CP_SOC_FIP_PFMID"]:
		cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{} \"echo overlake | sudo -S /usr/bin/{}\"".format(MBSNdict["cp_ip"], g_cmd_update_sop[0])
		print("[debug]g_cmd_update_sop cmd:{}".format(cmd))
		result = getresult(cmd)
		sendlog(result, 0, log_path)
		if result == "":
	...
	cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{} \"echo overlake | sudo -S /usr/bin/{}\"".format(MBSNdict["cp_ip"], g_cmd_update_sop[1])
	print("[debug]g_cmd_update_sop cmd:{}".format(cmd))
	result = getresult(cmd)
	sendlog(result, 0, log_path)
	if result == "":
	...
	#Check Nitro
	get_nitro_pfmid = ""
	cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{} \"echo overlake | sudo -S /usr/bin/{}\"".format(MBSNdict["cp_ip"],  g_cmd_cerberus_pfm_chk.format(1))
	print("[debug]g_cmd_update_sop cmd:{}".format(cmd))
	result = getresult(cmd)
	sendlog(result, 0, log_path)
	if result == "":
		...
		for line in result.splitlines():
			if  "Cerberus PFM ID:" in line:
				...
				break
	print("[debug]get_nitro_pfmid:{}".format(get_nitro_pfmid))
	print("[debug]CP_SOC_NITRO_PFMID:{}".format(MBSNdict["CP_SOC_NITRO_PFMID"]))
	if get_nitro_pfmid != MBSNdict["CP_SOC_NITRO_PFMID"]:
		cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{} \"echo overlake | sudo -S /usr/bin/{}\"".format(MBSNdict["cp_ip"], g_cmd_update_sop[2])
		print("[debug]g_cmd_update_sop cmd:{}".format(cmd))
		result = getresult(cmd)
		sendlog(result, 0, log_path)
		if result == "":
	...
	cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{} \"echo overlake | sudo -S /usr/bin/{}\"".format(MBSNdict["cp_ip"], g_cmd_update_sop[3])
	print("[debug]g_cmd_update_sop cmd:{}".format(cmd))
	result = getresult(cmd)
	sendlog(result, 0, log_path)
	if result == "":
	...
	if MBSNdict["CP_CERBERUS_FW"] != get_soc_cerberus_fw_ver:
		cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{} \"echo overlake | sudo -S /usr/bin/{}\"".format(MBSNdict["cp_ip"], g_cmd_update_sop[4])
		print("[debug]g_cmd_update_sop cmd:{}".format(cmd))
		result = getresult(cmd)
		sendlog(result, 0, log_path)
		if result == "":
...
def update_fpga_fw(MBSNdict):	  
	...
	if res != "":
		...
		sendlog("[SN:{}] Cannot find cp FPGA firmware file name".format(MBSNdict["mbsn"]))
		return 1
...
def is_CP_update(MBSNdict):
	...
	if not os.path.isfile(flag_path):
		...
		ret1 = chk_soc_fw(MBSNdict)
		ret2 = chk_fpga_fw(MBSNdict)
		if ret1 == 0 and ret2 == 0:
			os.system("touch {0}".format(flag_path))
			
			#set to emmc mode again to avoid CP card still in pxe boot mode. Jenny 20240926
			#Set bootmode to 0
			print ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Set bootmode to 0".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"]))
			cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{} \"echo overlake | sudo -S /usr/bin/{}\"".format(MBSNdict["cp_ip"], g_cmd_set_bootmode_to_zero)
			print("[debug]cmd:{}".format(cmd))
			...
			sendlog("[MBSN:{0}] SOC/FPGA firmware are not match.".format(MBSNdict["mbsn"]), RED)
			return 1
	return 0
	...
	if not os.path.isfile(updating_flag):
		...
		if ret == 0:
			...
			if auto_reconfig_python_soc_loc in result:
				...
				sendlog("SoC firmware is error and need to update", 0)
				ret = update_soc_fw(MBSNdict)
				if ret != 0:
					sendlog("[MBSN:{0}] update SoC firmware FAIL".format(MBSNdict["mbsn"]), RED)
				else:
...
def create_auto_reconfig_file(MBSNdict):
    ...
    cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{0} \"echo overlake | sudo -S {1}\"".format(MBSNdict["cp_ip"], g_cmd_find_auto_service)
    result = getresult(cmd)
    sendlog(result, 0, log_path)
    ...
    if auto_reconfig_service_soc_loc in result:
        ...
        cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{0} \"echo overlake | sudo -S {1}\"".format(MBSNdict["cp_ip"], g_cmd_find_auto_python)
        result = getresult(cmd)
        sendlog(result, 0, log_path)
        if auto_reconfig_python_soc_loc in result:
    ...
    cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{} \"echo overlake | sudo -S /sbin/ifconfig\"".format(MBSNdict["cp_ip"])
    result = getresult(cmd)
    sendlog(result, 0, log_path)
    ...
    cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{} \"echo overlake | sudo -S /usr/bin/{}\"".format(MBSNdict["cp_ip"], g_cmd_mkdir_service_loc_path)
    result = getresult(cmd).split('\n')
    sendlog(result, 0, log_path)
    if result == "":
    ...
    cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{0} \"echo overlake | sudo -S {1}\"".format(MBSNdict["cp_ip"], g_cmd_copy_auto_service_to_loc_path)
    result = getresult(cmd).split('\n')
    sendlog(result, 0, log_path)

    sendlog ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] copy  SoCFPGATestSvc.service file to CP console".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"]), 0, log_path)
    cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{0} \"echo overlake | sudo -S {1}\"".format(MBSNdict["cp_ip"], g_cmd_copy_auto_python_to_loc_path)
    result = getresult(cmd).split('\n')
    sendlog(result, 0, log_path)
...
if __name__ == "__main__":
	...
	if ret != 0:
		...
		for i in range(len(mbsn_info_list)):
			...
			sn_log_folder = "{0}/{1}".format(g_log_folder, mbsn)
			record_log_name = "{0}_do_record_CP_golden_image_version.log".format(mbsn)
			record_log_path = "{0}/{1}".format(sn_log_folder, record_log_name)
			if not os.path.isfile(record_log_path):
				print("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Remove and recreate SN folder".format(mbsn, cp_ip, cp_mac_addr))
				cmd = "rm -rf {}".format(sn_log_folder)
				sendlog(cmd, 0)
				getresult(cmd)

				print("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Remove auto reconfig flag".format(mbsn, cp_ip, cp_mac_addr))
				cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{0} \"echo overlake | sudo -S {1}\"".format(cp_ip, g_cmd_remove_auto_reconfig)
				sendlog(cmd, 0)
				result = getresult(cmd)
				sendlog(result, 0)
				cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{0} \"echo overlake | sudo -S sync\"".format(cp_ip)
				sendlog(cmd, 0)
				getresult(cmd)

				print("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Change FPGA slot to Golden".format(mbsn, cp_ip, cp_mac_addr))
				cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{} \"echo overlake | sudo -S /usr/sbin/modprobe catapult\"".format(cp_ip)
				sendlog(cmd, 0)
				getresult(cmd)
				cmd = "sshpass -p overlake ssh -o 'StrictHostKeyChecking no' -o UserKnownHostsFile=/dev/null stuser@{} \"echo overlake | sudo -S /usr/bin/fpgadiagnostics -reconfiggolden\"".format(cp_ip)
				sendlog(cmd, 0)
				result = getresult(cmd)
				sendlog(result, 0)
				time.sleep(30)

				mbsn_info_list[i]["is_reconfig"] = False
