...
g_switch_config_files = ["sonic-aboot-broadcom-20201231.66.swi", "SW_Config.sh"]
g_ycable_files = ["AEC_VERMONT_MV_1.0_20220907.bin","mux_update_T6G_A.py"]
g_script_folder = "/project/firmware/{0}/switch".format(g_project.lower())
...
def read_sn_info_from_config(sn):	
    ...
    for line in config:
        ...
        if line['Parameter'] == "PARTN" and line['Value'] != g_model:
            print("Wrong model")
            return 1
    for line in config:
...
def send_file_to_switch(ip,log):
    ...
    output = output + ret
    print(ret)
    cmd = g_sshpass_scp.format(g_switch_passwords[0],g_script_folder+"/*", ip, "/host/FW_Update/")
    ret = getresult(cmd)
    output = output + ret
    print(ret)
...
def send_mux_firmware_to_switch(ip,log):
    ...
    output = output + ret
    print(ret)
    cmd = g_sshpass_scp.format(g_switch_passwords[0],g_script_folder+"/*.bin", ip, "/host/FW_Update/")
    ret = getresult(cmd)
    output = output + ret
    print(ret)
    print("copying to switch")
    time.sleep(20)
    cmd = "sudo cp /host/FW_Update/*.bin /usr/share/sonic/firmware/"
    ret = getresult(g_sshpass_cmd.format(g_switch_passwords[0],ip,cmd))
    output = output + ret
    print(ret)
    with open(log, 'a') as file:
...
def check_switch_os_version(ip,log):
    ...
    with open(log, 'a') as file:
        # Write output to the file
        file.write(ret + "\n")
    if "SONiC Software Version: SONiC.20201231.66" not in ret:
...
def update_switch_os(ip,log):
    ...
    cmd = "sudo sonic-installer install /host/FW_Update/{0} -y".format(g_switch_config_files[0])
    ret = getresult(g_sshpass_cmd.format(g_switch_passwords[0],ip,cmd))
    time.sleep(40)
    print(ret)
    with open(log, 'a') as file:
...
def run_y_cable_update(ip,log):
    cmd = "sshpass -p 'Temp123!' ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null admin@{0} 'sudo python3 /host/FW_Update/{1}'".format(ip,g_ycable_files[1])
...
def config_switch(sn_info, switch):
    if switch == 0:
        ip = get_ip(sn_info["switch_a_mac"])
        tor = "A"
        log = "/RACKLOG/{2}/RM_logs/{0}/TOR_{1}.log".format(sn_info["RACKSN"],tor,g_project.lower())
    else:
        ip = get_ip(sn_info["switch_b_mac"])
        tor = "B"
        log = "/RACKLOG/{2}/RM_logs/{0}/TOR_{1}.log".format(sn_info["RACKSN"],tor,g_project.lower())
...
def auto_config_switches(sn_info):
    ...
    with open(updating_flag, 'w') as file:
        file.write("updating...")
    if not os.path.exists("/RACKLOG/{1}/RM_logs/{0}/".format(sn_info["sn"],g_project.lower())):
        os.makedirs("/RACKLOG/{1}/RM_logs/{0}/".format(sn_info["sn"],g_project.lower()))
    with open("/RACKLOG/{1}/RM_logs/{0}/{0}_updating.log".format(sn_info["sn"],g_project.lower()), 'w') as file:
        file.write("updating...")
    log_type="START"
    ...
    log_file =""
    send_data_sf(start_time,message, log_type, sn_info["sn"], sn_info["LOCATION"], sn_info["STATION"], log_file)
    if os.path.exists("/RACKLOG/{1}/RM_logs/{0}/{0}_updating.log".format(sn_info["sn"],g_project.lower())):
        os.remove("/RACKLOG/{1}/RM_logs/{0}/{0}_updating.log".format(sn_info["sn"],g_project.lower()))
    return 0
