...
# tftp_user = "test"
# tftp_passwd = "qcitest"
# cp_autoreconfig_ip = "192.168.1.202"
#########################
        ...
        if device == "CP":
            ...
            if ssh_object.before.decode ('utf-8') == "" or "@localhost:" not in ssh_object.before.decode ('utf-8').splitlines()[-1]:
                ...
                if ssh_object.before.decode ('utf-8') == "" or "@localhost:" not in ssh_object.before.decode ('utf-8').splitlines()[-1]:
                    ...
                    send_cmd_to_rm(ssh_object, sn_info_list, "set system reset -i {0}".format(sn_info_list["uut_to_rm_port"]), 5, log_path)
                    time.sleep(60) #for CP reset
                    return 1, None
        elif device == "RM":
...
def scp_send_cmd_to_cp(ssh_object, sn_info_list, cmd, sleep_time, log_path = "",device = "CP"):
    try:
        ...
        ssh_object.prompt()
        if device == "CP":
            while os.system(g_cmd_ping.format(sn_info_list["cp_ip"])) == 0: #Try to ping CP IP before scp copy. Jenny 20240913
                break
            if ssh_object.before.decode ('utf-8') == "" or "@localhost:" not in ssh_object.before.decode ('utf-8').splitlines()[-1]:
                #Login to RM switch
                #ssh_object = login_rm(sn_info_list)
...
def send_file_to_console(ip,from_file,to_file,sn_info_list):
    ...
    while "Another instance of" in  result_str:
        ...
        while os.system(g_cmd_ping.format(ip)) == 0: #Try to ping samba IP before scp copy. Jenny 20240913
            break
...
# 20241121 David add use WebAPI to query SF config
def getPXE():
    cmd = "ip a"
    global PXE
    res = getresult(cmd)
...
def getServers():
    ...
    for racksn in WIP:
        print("Check Rack {0}".format(racksn))
        client = Client(api_url)
        API_Type = "RLT"
        API_Station = 'QUERY'
        ...
        for i in data['Assets']['Asset']: 
            ...
            if (type != 'SERVERS'):
                continue
            unit_sn =  i['Serial']
            model = i['TestPosition'].upper()
            if unit_sn[0] == 'P' and g_model in model.upper():
                if unit_sn in serial_numbers:
                    continue
                serial_numbers.append(unit_sn)
    return (serial_numbers)
...
def getStation(sn):
    client = Client(api_url)
    API_Type = "SYSTEM"
    res = client.service.ReturnStatus(sn,API_Type)
    if (res['Msg'] != "OK"):
        print("API Query with Type {0} did not work. Retry with Type MB".format(API_Type))
        API_Type = "MB"
        res = client.service.ReturnStatus(sn,API_Type)
    if (res['Msg'] == "OK"):
        assets =  res['Asset']
        for item in assets:
            return item['Station'].upper()
    else:
        return "Unknow"
#*****************************************************************************
# Function	 : read_sn_info_from_api_config
# Description: Get all SN info from SF api config, rm table and DHCP
# Inputs	 : sn_info_list: an empty array for storing SN info
# Outputs	 : 0: PASS
#			 : 1: FAIL
# Notice	 : 20241121 David add use api to read SF config
#*****************************************************************************
def read_sn_info_from_api_config(sn):
    config = getConfig(sn)
    if config == 1:
        return 1
    SNdict = {
    ...
    for line in config:
        ...
        if "RACKPN" in line['Parameter']:
            rackpn = line['Value'].strip()
            
        #Get rm mac address from SN.txt
        if "RACK_MOUNT_MAC1" in line['Parameter']:
            rm_mac1 = line['Value'].strip()

        if "RACK_MOUNT1_MAC1" in line['Parameter']:
            rm_mac2 = line['Value'].strip()
            
        #Get uut mac address from SN.txt
        if "ETH0" in line['Parameter'] and "CP" not in line['Value']:
            nic_info = line['Value'].strip()
            nic_info = nic_info.split(',')
            if len(nic_info) != 0:
                SNdict["uut_mac_addr"] = nic_info[0].strip()
                
        if "ETH" in line['Parameter'] and "MICROSOFT" in line['Value'].upper():
            nic_info = line['Value'].strip()
            nic_info = nic_info.split(',')
            if len(nic_info) != 0:
        ...
        # if "STATION" in line['Parameter']:
        #     SNdict["STATION"] = line['Value'].strip()
        SNdict["STATION"] = getStation(sn)
        ...
        if "CP_GOLDEN_IMAGE" in line['Parameter']:
            SNdict["CP_GOLDEN_IMAGE"] = line['Value'].strip()
        
        #Get CP_FACTORY_IMAGE from SN.txt
        if "CP_FACTORY_IMAGE" in line['Parameter']:
            SNdict["CP_FACTORY_IMAGE"] = line['Value'].strip()
        
        #Get CP_SOC_FIP_FW from SN.txt
        if "CP_SOC_FIP_FW" in line['Parameter']:
            SNdict["CP_SOC_FIP_FW"] = line['Value'].strip()
        #Get CP_SOC_FIP_PFMID from SN.txt
        if "CP_SOC_FIP_PFMID" in line['Parameter']:
            SNdict["CP_SOC_FIP_PFMID"] = line['Value'].strip().lower()
        #Get CP_SOC_NITRO_FW from SN.txt
        if "CP_SOC_NITRO_FW" in line['Parameter']:
            SNdict["CP_SOC_NITRO_FW"] = line['Value'].strip()
        #Get CP_SOC_NITRO_PFMID from SN.txt
        if "CP_SOC_NITRO_PFMID" in line['Parameter']:
            SNdict["CP_SOC_NITRO_PFMID"] = line['Value'].strip().lower()

        if "CP_SOC_OS_FW" in line['Parameter']:
            SNdict["CP_SOC_OS_FW"] = line['Value'].strip()

        if "CP_SOC_DRIVER" in line['Parameter']:
            SNdict["CP_SOC_DRIVER"] = line['Value'].strip()

        if "CP_CERBERUS_FW" in line['Parameter']:
            SNdict["CP_CERBERUS_FW"] = line['Value'].strip()
        
        #Get RACK_MOUNT_FW from SN.txt
        if "RACK_MOUNT_FW" in line['Parameter']:
            SNdict["RACK_MOUNT_FW"] = line['Value'].strip()
        
        #Get CSN from SN.txt
        if "CSN" in line['Parameter']:
            SNdict["csn"] = line['Value'].strip()
            print ("CSN:{0}".format(SNdict["csn"]))

        if "PDNUM" in line['Parameter']:
            SNdict["pdnum"] = line['Value'].strip()
            print ("PDNUM:{0}".format(SNdict["pdnum"]))
        
        #Get CP_SN from SN.txt. T6H is FPGASN. Jenny 20240913
        if "FPGASN" in line['Parameter']:
            SNdict["CP_SN"] = line['Value'].strip()
        if "CP_SN" in line['Parameter']:
            SNdict["CP_SN"] = line['Value'].strip()
...
def get_ip(mac_addr):
    ...
    if build_side == "QTMC" or build_side == "QMF" or build_side == "QCI":
...
#*****************************************************************************
# Function	 : chk_record_cp_and_reconfig
# Description: Step 1. Check App mode of fpga version is correct or not
#            : Step 2. Check the do_record_cp_golden_image_version.log file is correct or not.
#			 : Step 3. If step1 and step2 are correct, create flag and no need check again.
#            : Step 4. If step1 failed, remove <CP_SN>_update_flag and remove golden image file.
#            : Step 5. If step2 failed, remove remove golden image file.
# Inputs	 : sn_info_list: SN list information 
# Outputs	 : 0: PASS
#			 : 1: FAIL
# Notice	 : 
#*****************************************************************************
def chk_record_cp_and_reconfig(sn_info_list):
    
    #1. check app mode fpga firmware version
    #2. check do_record_cp_golden_image_version file informatin.

    sn_log_folder = "{0}/{1}".format(g_log_folder, sn_info_list["sn"])
    log_name = "{0}_chk_fpga_fw.log".format(sn_info_list["sn"])
    log_path = "{0}/{1}".format(sn_log_folder, log_name)
    fpga_update_flag = "{0}/{1}_update_flag.log".format(sn_log_folder, sn_info_list["CP_SN"])
    cp_goldem_image_log = "{0}/{1}_do_record_cp_golden_image_version.log".format(sn_log_folder, sn_info_list["sn"])
    chk_golden_file_flag = "{0}/{1}_chk_cp_golden_flag.log".format(sn_log_folder, sn_info_list["sn"])

    is_need_power_cycle = False
    ...
    if ret != 0:
        ...
        get_app_mode_fpga_ver = ver+','+role
        sendlog("[RACKSN:{0} SN:{1}] get_app_mode_fpga_ver = {2}".format(sn_info_list["RACKSN"], sn_info_list["sn"], get_app_mode_fpga_ver))
        
    if sn_info_list["CP_FACTORY_IMAGE"] != get_app_mode_fpga_ver:
        sendlog("[RACKSN:{0} SN:{1}] Remove {2}_update_flag.log and {2}_AutoReconfig_flag.log file".format(sn_info_list["RACKSN"], sn_info_list["sn"], sn_info_list["CP_SN"]))
        if os.path.isfile(fpga_update_flag):
            os.remove(fpga_update_flag)
        if os.path.isfile(cp_goldem_image_log):
            os.remove(cp_goldem_image_log)
        is_need_power_cycle = True

    sendlog("[RACKSN:{0} SN:{1}] get_app_mode_fpga_ver = {2}, expect = {3} PASS".format(sn_info_list["RACKSN"], sn_info_list["sn"], get_app_mode_fpga_ver, sn_info_list["CP_FACTORY_IMAGE"]))
    
    #Check do_record_cp_golden_image_version.log file
    if os.path.isfile(cp_goldem_image_log):
        golen_image_role_id = ""
        golden_image_version = ""
        golden_image_build_version = ""
        get_golden_mode_fpga_ver = ""
        f = open(cp_goldem_image_log)
        for line in f:
            if "Read register 101" in line:
                golen_image_role_id = line.split(" ")[5].strip()
            if "Read register 65" in line:
                golden_image_version = line.split(" ")[5].strip()
            if "Read register 59" in line:
                golden_image_build_version = line.split(" ")[5].strip()
            if "[FPGA-CONFIG-EX ] OK " in line:
                ver = line.split()[6].split(',')[1].split('-')[0].strip()
                role = line.split()[7].split(',')[0].split('role:')[1]
                get_golden_mode_fpga_ver = ver + ',' + role
                sendlog("[RACKSN:{0} SN:{1}] get_fpga_version ver:{2}, role:{3}".format(sn_info_list["RACKSN"],sn_info_list["sn"], ver, role))
        f.close()

        if golen_image_role_id == "" or golden_image_version == "" or golden_image_build_version == "":
            sendlog("[RACKSN:{0} SN:{1}] Wrong glden image value in file. Remove {2}_do_record_cp_golden_image_version.log file".format(sn_info_list["RACKSN"], sn_info_list["sn"], sn_info_list["sn"]))
            if os.path.isfile(cp_goldem_image_log):
                os.remove(cp_goldem_image_log)
            is_need_power_cycle = True
        
        sendlog("[RACKSN:{0} SN:{1}]Check golden file pass. golen_image_role_id:{2},golden_image_version{3}, golden_image_build_version:{4}".format(sn_info_list["RACKSN"], sn_info_list["sn"], golen_image_role_id,golden_image_version, golden_image_build_version))

        if sn_info_list["CP_GOLDEN_IMAGE"] != get_golden_mode_fpga_ver:
            sendlog("[RACKSN:{0} SN:{1}] Wrong golden image version in file. Remove {2}_do_record_cp_golden_image_version.log file".format(sn_info_list["RACKSN"], sn_info_list["sn"], sn_info_list["sn"]))
            if os.path.isfile(cp_goldem_image_log):
                os.remove(cp_goldem_image_log)
            is_need_power_cycle = True
        sendlog("[RACKSN:{0} SN:{1}]get_golden_mode_fpga_ver = {2}, expect = {3} PASS".format(sn_info_list["RACKSN"], sn_info_list["sn"], get_golden_mode_fpga_ver, sn_info_list["CP_GOLDEN_IMAGE"]))

    
    if is_need_power_cycle == True:
        #Remove auto reconfig file
        if remove_auto_reconfig_file(sn_info_list) != 0:
            msg = "remove_auto_reconfig_file FAIL !"
            sendlog("[RACKSN:{0} SN:{1}] {2}".format(sn_info_list["RACKSN"],sn_info_list["sn"], msg))
            return 1
        #set system reset
        sendlog("[RACKSN:{0} SN:{1}] Reset system after remove CP card flag file".format(sn_info_list["RACKSN"], sn_info_list["sn"]))
        send_cmd_to_rm(ssh_object, sn_info_list, "set system reset -i {0}".format(sn_info_list["uut_to_rm_port"]), 5, log_path)
        time.sleep(10)
        return 1

    sendlog("[RACKSN:{0} SN:{1}] Create {2} file".format(sn_info_list["RACKSN"], sn_info_list["sn"], chk_golden_file_flag))
    os.system("touch {0}".format(chk_golden_file_flag))
    return 0
...
def chk_soc_fw(sn_info_list):
    ...
    try:
        ...
        if sn_info_list["CP_CERBERUS_FW"] != get_soc_cerberus_fw_ver:
            ...
            sn_info_list["is_need_cerberus_update"] = True
...
def update_soc_fw(sn_info_list):	  
    ...
    if ret != 0:
        ...
        if res != "":
            if is_success(res):
                ...
                send_cmd_to_rm(ssh_object, sn_info_list, "set system boot -t eMMC -i {0} -b 1".format(sn_info_list["uut_to_rm_port"]), 5, log_path)
                send_cmd_to_rm(ssh_object, sn_info_list, "set system remotedrive unmount -i {0} -b 1".format(sn_info_list["uut_to_rm_port"]), 5, log_path)
                send_cmd_to_rm(ssh_object, sn_info_list, "set manager port off -i {0}".format(sn_info_list["uut_to_rm_port"]), 5, log_path)
                time.sleep(30) #for AC OFF
                send_cmd_to_rm(ssh_object, sn_info_list, "set manager port on -i {0}".format(sn_info_list["uut_to_rm_port"]), 5, log_path)
    ...
    ret, res = send_cmd_to_cp(ssh_object, sn_info_list, g_cmd_socflash, 10*60, log_path)
    if ret != 0:
        ret, res = send_cmd_to_cp(ssh_object, sn_info_list, g_cmd_set_bootmode_to_zero, 0, log_path)
        return 1
    ...
    g_cmd_update_sop = ["cerberus_utility pfmupdate 0 /tmp/usb/{} 0".format(fip_pfm_file),
                        ...
                        "cerberus_utility fwupdate /tmp/usb/{}".format(g_cp_Cerberus_fw)]
    ...
    # if sn_info_list["is_need_cerberus_update"]:
    cmd = g_cmd_update_sop[4]
    ret, res = send_cmd_to_cp(ssh_object, sn_info_list, cmd, 120, log_path)
    if ret != 0:
        ret, res = send_cmd_to_cp(ssh_object, sn_info_list, g_cmd_set_bootmode_to_zero, 0, log_path)
        return 1
    ...
    '''
    #Get Expected Cerberus firmware md5sum. Jenny 20240917
    expected_cerberus_md5sum = ""
    cmd = "md5sum {}".format(g_cp_Cerberus_fw_loc)
    res = getresult(cmd)
    if res != "":
        expected_cerberus_md5sum = res.split()[0].strip()
    else:
        sendlog("[RACKSN:{0} SN:{1}] Cannot find Cerberus firmware md5sum".format(sn_info_list["RACKSN"], sn_info_list["sn"]), RED)
        return 1
    sendlog("[RACKSN:{0} SN:{1}] Expect {2} md5sum: {3}".format(sn_info_list["RACKSN"], sn_info_list["sn"],g_cp_Cerberus_fw_loc, expected_cerberus_md5sum))

    #Cerberus firmware file copy and check Cerberus md5sum. Jenny 20240917
    get_cerberus_md5sum = ""
    while get_cerberus_md5sum != expected_cerberus_md5sum:
        ...
        time.sleep(5)
        cmd = "md5sum {}".format(g_cp_tmp + g_cp_Cerberus_fw)
        ret, res = send_cmd_to_cp(ssh_object, sn_info_list, cmd, 2, log_path)
        if ret != 0:
            ret, res = send_cmd_to_cp(ssh_object, sn_info_list, g_cmd_set_bootmode_to_zero, 0, log_path)
            return 1
        else:
            if res != "":
                for line in res.splitlines():
                    if g_cp_Cerberus_fw in line:
                        get_cerberus_md5sum = line.split()[0].strip()
                        break
            else:
                sendlog("[RACKSN:{0} SN:{1}] Can not get result - Cerberus firmware md5sum result".format(sn_info_list["RACKSN"], sn_info_list["sn"]))
                ret, res = send_cmd_to_cp(ssh_object, sn_info_list, g_cmd_set_bootmode_to_zero, 0, log_path)
                return 1
        sendlog("[RACKSN:{0} SN:{1}] Get {2} md5sum: {3}".format(sn_info_list["RACKSN"], sn_info_list["sn"],g_cp_Cerberus_fw, get_cerberus_md5sum))
    ...
    #Set bootmode to 0
    send_cmd_to_cp(ssh_object, sn_info_list, g_cmd_set_bootmode_to_zero, 5, log_path)
    ret, res = send_cmd_to_cp(ssh_object, sn_info_list, g_cmd_get_bootmode, 5, log_path)
    if ret != 0:
        ret, res = send_cmd_to_cp(ssh_object, sn_info_list, g_cmd_set_bootmode_to_zero, 5, log_path)
        return 1
    else:
        if res != "":
            if "bootmode = 0x00" in res:
...
def first_update_soc_fw(sn_info_list):	  
    ...
    try:
        ...
        if ret != 0:
            raise Exception()
        fip_fw_file = re.search("fip.*?bin", res).group()
    except:
        fip_fw_file = "fip_padded.bin"
    ...
    #Copy image from TFTP to CP SoC
    ret, res = scp_send_cmd_to_cp(ssh_object, sn_info_list, g_cmd_copy_image_to_soc, 60, log_path)
    if ret != 0:
        return 1
    else:
        if res != "":
...
def update_fpga_fw(sn_info_list):
    ...
    #Get Expected FPGA firmware md5sum. Jenny 20240917
    expected_fpga_md5sum = ""
    cmd = "md5sum {}".format(g_cp_FPGA_firmware_loc)
    res = getresult(cmd)
    if res != "":
        expected_fpga_md5sum = res.split()[0].strip()
    else:
        sendlog("[RACKSN:{0} SN:{1}] Cannot find FPGA firmware md5sum".format(sn_info_list["RACKSN"], sn_info_list["sn"]), RED)
        return 1
    sendlog("[RACKSN:{0} SN:{1}] Expect {2} md5sum: {3}".format(sn_info_list["RACKSN"], sn_info_list["sn"],g_cp_FPGA_firmware, expected_fpga_md5sum))
    ...
    #FPGA firmware file copy and check FPGA md5sum. Jenny 20240917
    get_fpga_md5sum = ""
    while get_fpga_md5sum != expected_fpga_md5sum:
        #Copy image to CP SOC
        sendlog("[RACKSN:{0} SN:{1}] Copy {2} to CP {3}".format(sn_info_list["RACKSN"], sn_info_list["sn"], g_cp_FPGA_firmware_loc,g_cp_tmp))    
        g_cmd_copy_image_to_soc = "scp {0}@{1}:{2} {3}".format(tftp_user,tftp_ip,g_cp_FPGA_firmware_loc,g_cp_tmp)
        #send_file_to_console(CP_IP,g_cp_FPGA_firmware_loc,g_cp_tmp,sn_info_list)
        ...
        if ret != 0:
            return 1
        time.sleep(2)
        cmd = "md5sum {}".format(g_cp_tmp + g_cp_FPGA_firmware)
        ret, res = send_cmd_to_cp(ssh_object, sn_info_list, cmd, 3, log_path)
        if ret != 0:
            return 1
        else:
            if res != "":
                for line in res.splitlines():
                    if g_cp_FPGA_firmware in line:
                        get_fpga_md5sum = line.split()[0].strip()
                        break
            else:
                sendlog("[RACKSN:{0} SN:{1}] Can not get result - FPGA firmware md5sum result".format(sn_info_list["RACKSN"], sn_info_list["sn"]))
                return 1
        sendlog("[RACKSN:{0} SN:{1}] Get {2} md5sum: {3}".format(sn_info_list["RACKSN"], sn_info_list["sn"],g_cp_FPGA_firmware, get_fpga_md5sum))
...
def remove_auto_reconfig_file(sn_info_list):
    ...
    log_path = "{0}/{1}".format(sn_log_folder, log_name)
    auto_reconfig_flag = "{0}/{1}_AutoReconfig_flag.log".format(sn_log_folder,sn_info_list["CP_SN"]) # 20240916 Jenny add auto reconfig flag
    ...
    #Remove auto reconfig flag file.
    if os.path.isfile(auto_reconfig_flag):
        os.remove(auto_reconfig_flag)
...
def create_auto_reconfig_file(sn_info_list):
    ...
    #-----------
    #Get Expected reconfig service file md5sum. Jenny 20240917
    expected_ReconServ_md5 = ""
    cmd = "md5sum {}".format(auto_reconfig_service_server_loc)
    res = getresult(cmd)
    if res != "":
        expected_ReconServ_md5 = res.split()[0].strip()
    else:
        sendlog("[RACKSN:{0} SN:{1}] Cannot find auto reconfig service file md5sum".format(sn_info_list["RACKSN"], sn_info_list["sn"]), RED)
        return 1
    sendlog("[RACKSN:{0} SN:{1}] Expect {2} md5sum: {3}".format(sn_info_list["RACKSN"], sn_info_list["sn"],auto_reconfig_service_server_loc, expected_ReconServ_md5))

    #reconfig service file copy and check md5sum. Jenny 20240917
    get_ReconServ_md5 = ""
    while get_ReconServ_md5 != expected_ReconServ_md5:
        #Copy image to CP SOC
        sendlog("[RACKSN:{0} SN:{1}] Copy {2} to CP {3}".format(sn_info_list["RACKSN"], sn_info_list["sn"], auto_reconfig_service_server_loc,g_cp_tmp))    
        g_cmd_copy_file_to_soc = "scp {0}@{1}:{2} {3}".format(tftp_user,tftp_ip,auto_reconfig_service_server_loc,g_cp_tmp)
        if build_side == "QCI":
            g_cmd_copy_file_to_soc = "scp {0}@{1}:{2} {3}".format(tftp_user,CP_IP,auto_reconfig_service_server_loc,g_cp_tmp)
        #Copy image from TFTP to CP SoC
        ret, res = scp_send_cmd_to_cp(ssh_object, sn_info_list, g_cmd_copy_file_to_soc, 20, log_path)
        if ret != 0:
            return 1
        time.sleep(2)
        cmd = "md5sum {}".format(g_cp_tmp + os.path.basename(auto_reconfig_service_server_loc))
        ret, res = send_cmd_to_cp(ssh_object, sn_info_list, cmd, 3, log_path)
        if ret != 0:
            return 1
        else:
            if res != "":
                for line in res.splitlines():
                    if os.path.basename(auto_reconfig_service_server_loc) in line and "tee" not in line and "systemd" not in line and len(line.split()) == 2:
                        get_ReconServ_md5 = line.split()[0].strip()
                        break
            else:
                sendlog("[RACKSN:{0} SN:{1}] Can not get result - FPGA firmware md5sum result".format(sn_info_list["RACKSN"], sn_info_list["sn"]))
                return 1
        sendlog("[RACKSN:{0} SN:{1}] Get {2} md5sum: {3}".format(sn_info_list["RACKSN"], sn_info_list["sn"],os.path.basename(auto_reconfig_service_server_loc), get_ReconServ_md5))
    #-----------
    #-----------
    #Get Expected reconfig service file md5sum. Jenny 20240917
    expected_ReconFPGA_md5 = ""
    cmd = "md5sum {}".format(auto_reconfig_python_server_loc)
    res = getresult(cmd)
    if res != "":
        expected_ReconFPGA_md5 = res.split()[0].strip()
    else:
        sendlog("[RACKSN:{0} SN:{1}] Cannot find auto reconfig fpga file md5sum".format(sn_info_list["RACKSN"], sn_info_list["sn"]), RED)
        return 1
    sendlog("[RACKSN:{0} SN:{1}] Expect {2} md5sum: {3}".format(sn_info_list["RACKSN"], sn_info_list["sn"],auto_reconfig_service_server_loc, expected_ReconFPGA_md5))

    #reconfig service file copy and check md5sum. Jenny 20240917
    get_ReconFPGA_md5 = ""
    while get_ReconFPGA_md5 != expected_ReconFPGA_md5:
        #Copy image to CP SOC
        sendlog("[RACKSN:{0} SN:{1}] Copy {2} to CP {3}".format(sn_info_list["RACKSN"], sn_info_list["sn"], auto_reconfig_python_server_loc,g_cp_tmp))    
        g_cmd_copy_file_to_soc = "scp {0}@{1}:{2} {3}".format(tftp_user,tftp_ip,auto_reconfig_python_server_loc,g_cp_tmp)
        if build_side == "QCI":
            g_cmd_copy_file_to_soc = "scp {0}@{1}:{2} {3}".format(tftp_user,CP_IP,auto_reconfig_python_server_loc,g_cp_tmp)
        #Copy image from TFTP to CP SoC
        ret, res = scp_send_cmd_to_cp(ssh_object, sn_info_list, g_cmd_copy_file_to_soc, 20, log_path)
        if ret != 0:
            return 1
        time.sleep(2)
        cmd = "md5sum {}".format(g_cp_tmp + os.path.basename(auto_reconfig_python_server_loc))
        ret, res = send_cmd_to_cp(ssh_object, sn_info_list, cmd, 3, log_path)
        if ret != 0:
            return 1
        else:
            if res != "":
                for line in res.splitlines():
                    if os.path.basename(auto_reconfig_python_server_loc) in line and "tee" not in line and "systemd" not in line and len(line.split()) == 2:
                        get_ReconFPGA_md5 = line.split()[0].strip()
                        break
            else:
                sendlog("[RACKSN:{0} SN:{1}] Can not get result - FPGA firmware md5sum result".format(sn_info_list["RACKSN"], sn_info_list["sn"]))
                return 1
        sendlog("[RACKSN:{0} SN:{1}] Get {2} md5sum: {3}".format(sn_info_list["RACKSN"], sn_info_list["sn"],os.path.basename(auto_reconfig_python_server_loc), get_ReconFPGA_md5))
...
def check_cp_sn_match_with_sf(sn_info_list): #20241111 david
    #print("CMD:" + g_cmd_RM_API_FPGA_info.format(sn_info_list["rm_ip"], sn_info_list["uut_to_rm_port"]))
    ret,output,stderr = gs_exec_cmd(g_cmd_RM_API_FPGA_info.format(sn_info_list["rm_ip"], sn_info_list["uut_to_rm_port"]))
    print(output)
    time.sleep(1)
    is_find_str = False
    if ret == 0:
        for line in output.splitlines():
            ...
            if "SerialNumber" in line:
                is_find_str = True
                if sn_info_list["CP_SN"] in line:
                    sendlog("[RACKSN:{0} SN:{1}] CP SN Match with SF".format(sn_info_list["RACKSN"],sn_info_list["sn"]))
                    return 0
                else:
                    sendlog("[RACKSN:{0} SN:{1}] CP SN Not Match with SF, GET: {}".format(sn_info_list["RACKSN"],sn_info_list["sn"], line))
                    return 2
    if is_find_str == False:
        sendlog ("[RACKSN:{0} SN:{1}] Cannot find CP SerialNumber string".format(sn_info_list["RACKSN"],sn_info_list["sn"]))
        return 1
    ...
    if sn_info_list != 1:
        ...
        if sn_info_list["is_reconfig"]:
            if not os.path.isfile(update_flag):
                ...
                #Check do_record_cp_golden_image_version file in PRETEST 
                if sn_info_list["STATION"] == "PRETEST" and not os.path.isfile(chk_golden_file_flag):
                    if chk_record_cp_and_reconfig(sn_info_list) != 0:
                        sendlog("[RACKSN:{0} SN:{1}] chk_record_cp_and_reconfig FAIL !".format(sn_info_list["RACKSN"],sn_info_list["sn"]))
                        clear_flag(sn_info_list)
                        return 1
                return 0
        ...
        #if check_cp_sn_match_with_sf(sn_info_list) != 0: # 20241111 David add CP SN check
        #    return 1
        try:
            if not os.path.isfile(updating_flag):
                ...
                if not os.path.isfile(update_flag):
                    if sn_info_list["STATION"] == "PRETEST" or sn_info_list["STATION"] == "TOR":
                        ...
                        #Checp CP IP address first. Jenny 20240917
                        if sn_info_list["cp_ip"] == 1:
                            msg = "Cannot get CP IP address"
                            send_data_sf(time.strftime("%Y%m%d%H%M%S"),msg, "WARNING", sn_info_list["sn"], sn_info_list["LOCATION"], sn_info_list["STATION"], "")
                            clear_flag(sn_info_list)
                            return 1
            ...
            print("sn_info_list = {}".format(sn_info_list))
            print("update_flag = {}".format(update_flag))
            if not os.path.isfile(updating_flag):
                os.system("touch {0}".format(updating_flag))
                #Check reconfig flag
                if not os.path.isfile(golden_log_path) and sn_info_list["STATION"] == "PRETEST":
                ...
                # Use auto reconfig flag to check CP is already reconfig or not. Jenny 20240916 
                if not os.path.isfile(auto_reconfig_flag): 
                    ret = create_auto_reconfig_file(sn_info_list)
                    if ret == 1:
                        sendlog("[RACKSN:{0}, SN: {1}] create auto reconfig file FAIL".format(sn_info_list["RACKSN"], sn_info_list["sn"]), RED)
                        os.system("rm -rf {0}".format(updating_flag))
                        return 1

                    if (sn_info_list["STATION"] == "PRETEST" or sn_info_list["STATION"] == "TOR") and ret == 2:
                            log_type="PASS"
                            message = "CP reconfig FINISH !"
                            start_time=time.strftime("%Y%m%d%H%M%S")
                            log_file =""
                            send_data_sf(start_time,message, log_type, sn_info_list["sn"], sn_info_list["LOCATION"], sn_info_list["STATION"], log_file)
                            os.system("touch {0}".format(auto_reconfig_flag))
            ...
            sendlog("[RACKSN:{0} SN:{1}] Function exception, remove update flag.".format(sn_info_list["RACKSN"], sn_info_list["sn"]))
            os.system("rm -rf {0}".format(updating_flag))
...
if __name__ == "__main__":
    ...
    """ # Dylan, 2024/11/02, Mask T6G auto reconfig part.
    ...
    if build_side != "QMF":			
        while True:
            ...
            print("sf_file_list = {}".format(sf_file_list)) # Dylan, 2024/11/04, add for debug.
        ...
        if g_api_sf:
            getPXE()
            serial_numbers = getServers()
            print("sn_list = {}".format(serial_numbers))
            for i in range(len(serial_numbers)):
                # p = Process(target=check_sf_and_soc_fw_chk_and_reconfig_cp_chk, args=(serial_numbers[i],))
                p = Process(target=check_sf_and_soc_fw_chk_and_reconfig_cp_chk, args=(serial_numbers[i]+'.txt',))
                if p != "":
                    p.start()
                if i != 0 and i%11 == 0: #20230829 David add prevent server busy
                    time.sleep(10)
