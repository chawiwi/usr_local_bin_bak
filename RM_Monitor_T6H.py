...
def do_AC_Cycle(rm_ip, location):
	...
	if port != FAIL and port != "":
		...
		do_ssh_connect(cmd,rm_ip, g_rm_pass)
		time.sleep(10)
		ret = check_pwr_status_on(rm_ip,port)
		if ret != 0:
			for i in range(3):
				cmd = "set manager port {0} -i {1}".format("ON", port)
				do_ssh_connect(cmd, rm_ip, g_rm_pass)
				time.sleep(10)
				ret = check_pwr_status_on(rm_ip, port)
				if ret == 0:
					break
				if i == 2:
					logging.error( "RM power on UUT-{0} FAIL".format(port))
					return 1
		return 0
...
def check_pwr_status_off(rm_ip, locate_idx):
	...
	if "No route to host" in stdout:#20240731 Wayne
		time.sleep(15)
	if "Completion Code:" in stdout:#20240731 Wayne prevent next line IndexError: list index out of range
		cmplt_code = stdout.split("Completion Code:")[-1].splitlines()[0].strip()
					...
					if ".txt" in get_info :  
							...
							if req != 1 and sf != 1:
									...
									if action == ACTION_AC_CYCLE:
											sku_info = getSku()
											#=== Get master node count in config_skuX.ini ====
											ccr_config_file = "{0}config_{1}.ini".format(ccr_config,sku_info)
											if not os.path.isfile(ccr_config_file):
												log = "RM config file [{0}]doesn\'t exist.".format(cable_mapping)
												logging.error(log, FAIL)

											f = open(ccr_config_file)
											for line in f:
												if "node =" in line:
											...
											if len(master_node) == 2:
												if location >= "F27":
													rm_mac = get_single_info(sf,"RACK_MOUNT1_MAC1")
												elif location <= "F20":
