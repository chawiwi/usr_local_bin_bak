...
def gs_get_rm_port(ip,MBSNdict):
    ...
    pxe_loc = int(MBSNdict["TESTLOC"][1:3])
    uut_loc = int(MBSNdict["TESTLOC"][-2:])
    print (f"pxe_loc={pxe_loc}, uut_loc={uut_loc}")
    ...
    # "/RACKLOG/t6k/RM_logs/monitor.log"
    # if os.path.isfile(rm_file) and i_cfg.prj_build_site != "QCI":
    # if os.path.isfile(rm_file):
    #     os.system("rm -rf {}".format(rm_file))
    ...
    for line in rm_info:
        ...
        if 1:
            ...
            if pxe_loc % 2 == 0:
                server_loc = uut_loc + 25
            else:        
                server_loc =  uut_loc + 6
...
def update_soc_fw(MBSNdict):
    ...
    if build_side == 'QCI':
        ...
        os.system("echo 'label T6K rsa image' > {}".format(image_mac_file))
