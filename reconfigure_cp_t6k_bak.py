...
g_cp_Cerberus_fw="cerberus_v2.4.12.1.bin"
...
# fip_pfm_file  = "A2040.FIP.PFM.42.bin"
fip_fw_file = "fip.bin"
# nitro_pfm_file = "A2040.NITRO.PFM.42.bin"
...
def update_soc_fw(MBSNdict):
	...
	if build_side == 'QCI':
		...
		print("building site other than QCI, image_path:{}".format(image_path))
	...
	print('Preparing to update the soc fw------------')
	cp_mac = MBSNdict["cp_mac_addr"].lower()
	print("cp_mac:{}".format(cp_mac))
	...
	if build_side == 'QCI':
		...
		print("createing image mac file in update_soc_fw")
...
def is_CP_update(MBSNdict):
	print("entering is_CP_update")
	sn_log_folder = "{0}/{1}".format(g_log_folder, MBSNdict["mbsn"])
	update_flag = "{0}_already_update.log".format(MBSNdict["mbsn"])
...
def do_record_CP_and_reconifg(MBSNdict):
	...
	if not os.path.isfile(updating_flag):
		...
		try:
			...
			print("cp_updat return: {}".format(ret))
			if ret == 0:
			...
			sendlog ("[mbsn: {0}, cp_ip: {1}] CP FW check/update Exception".format(MBSNdict["mbsn"], MBSNdict["cp_ip"]))
			print (sys.exc_info())
			clear_flag(MBSNdict)
			return 1
	...
	if ret != 0:
		...
		for i in range(len(mbsn_info_list)):
			...
			if not mbsn_info_list[i]["is_reconfig"]:
				...
				#do_record_CP_and_reconifg(mbsn_info_list[i],)
