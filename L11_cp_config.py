...
def login_rm(sn_info_list):
	try:
		...
		sendlog("[rm_ip:{0:>12} rm_mac: {1} RACKSN:{2} SN:{3}] login failed, delay 5s and retry".format(sn_info_list["rm_ip"], sn_info_list["rm_mac_addr"],sn_info_list["RACKSN"],sn_info_list["sn"]))
		time.sleep(5)
		try:
			ssh_object = pxssh.pxssh()
...
def send_cmd_to_rm(ssh_object, sn_info_list, cmd, sleep_time, log_path = "",device = "RM"):
	try:
		...
		ssh_object.buffer = b"" #20230724 david
		sendlog("[rm_ip:{0} rm_mac: {1} RACKSN:{2} SN:{3} {4}] cmd:{5}".format(sn_info_list["rm_ip"], sn_info_list["rm_mac_addr"], sn_info_list["RACKSN"],sn_info_list["sn"], device, cmd))
		# ssh_object.expect(r'.+')
		...
		sendlog("[rm_ip:{0} rm_mac: {1} RACKSN:{2} SN:{3} {4}] pxssh failed: can not execute cmd:{5}".format(sn_info_list["rm_ip"], sn_info_list["rm_mac_addr"], sn_info_list["RACKSN"],sn_info_list["sn"], device, cmd), RED)
		sendlog(e, RED)
		return 1, None
...
def do_reconfig_connect_rm_port(sn_info_list):
	...
	sendlog("[rm_ip:{0:>12} rm_mac: {1} rm_port: {2} RACKSN:{3} SN:{4}]CP Reconfig success".format(sn_info_list["rm_ip"], sn_info_list["rm_mac_addr"], sn_info_list["uut_to_rm_port"],sn_info_list["RACKSN"],sn_info_list["sn"]))
	return 0
...
def do_record_cp_golden_img_ver_connect_rm_port(sn_info_list):
	...
	sendlog("[rm_ip:{0} rm_mac: {1} rm_port: {2} RACKSN:{3} SN:{4}] golen_image_role_id = {5}".format(sn_info_list["rm_ip"], sn_info_list["rm_mac_addr"], sn_info_list["uut_to_rm_port"], sn_info_list["RACKSN"],sn_info_list["sn"], golen_image_role_id), 0, log_path)
	sendlog("[rm_ip:{0} rm_mac: {1} rm_port: {2} RACKSN:{3} SN:{4}] golden_image_version = {3}".format(sn_info_list["rm_ip"], sn_info_list["rm_mac_addr"], sn_info_list["uut_to_rm_port"], sn_info_list["RACKSN"],sn_info_list["sn"], golden_image_version), 0, log_path)
	sendlog("[rm_ip:{0} rm_mac: {1} rm_port: {2} RACKSN:{3} SN:{4}] golden_image_build_version = {3}".format(sn_info_list["rm_ip"], sn_info_list["rm_mac_addr"], sn_info_list["uut_to_rm_port"], sn_info_list["RACKSN"],sn_info_list["sn"], golden_image_build_version), 0, log_path)
...
def do_record_cp_and_reconfig(sn_info_list):
	...
	if rm_fpga_health_is_cp_golden_mode(sn_info_list) == 0:
		return 0
	if cp_ip_health_is_cp_golden_mode(sn_info_list) == 0:
		return 0
	if is_cp_golden_mode(sn_info_list):
		...
		# 	sendlog("[RACKSN:{0} SN: {1}] reconfig CP FAIL".format(sn_info_list["RACKSN"], sn_info_list["sn"]), RED)
		# 	return 1
	return 0
	...
	if ret != 0:
		...
		if res != "":
			if upgrade_image in res:
				sendlog("[RACKSN:{0} SN:{1}] {2} image from TFTP to RM success !".format(sn_info_list["RACKSN"],sn_info_list["sn"], upgrade_image))
			else:
...
def chk_fpga_fw(sn_info_list):
	...
	if ret != 0:
		...
		sendlog("[RACKSN:{0} SN:{1}] GOLDEN:get_fpga_fw_ver = {0}".format(sn_info_list["RACKSN"], sn_info_list["sn"], get_fpga_fw_ver))
...
def create_auto_reconfig_file(sn_info_list):
	...
	log_name = "{0}_cp_auto_reconfig.log".format(sn_info_list["sn"])
	log_path = "{0}/{1}".format(sn_log_folder, log_name)
	finish_flag_name = "{0}_test_finish.log".format(sn_info_list["sn"])
	finish_flag_path = "{0}/{1}".format(sn_log_folder, finish_flag_name)
	CP_IP=""

	if os.path.isfile(finish_flag_path):
		sendlog("[RACKSN:{0} SN:{1}] {2} flag exist, not create auto reconfig!!".format(sn_info_list["RACKSN"],sn_info_list["sn"],finish_flag_path))
		return 0	
	
	if not os.path.isfile(auto_reconfig_service_server_loc):
		sendlog("[RACKSN:{0} SN:{1}] File {2} not exist, please check it!!".format(sn_info_list["RACKSN"],sn_info_list["sn"],auto_reconfig_service_server_loc))
		return 1
	
	if not os.path.isfile(auto_reconfig_python_server_loc):
		sendlog("[RACKSN:{0} SN:{1}] File {2} not exist, please check it!!".format(sn_info_list["RACKSN"],sn_info_list["sn"],auto_reconfig_python_server_loc))
		return 1
	...
	if sn_info_list != 1:
		...
		if not os.path.isfile(updating_flag):
			...
			if not os.path.isfile(update_flag):
				if sn_info_list["STATION"] == "PRETEST" or sn_info_list["STATION"] == "TOR":
					...
					if remove_auto_reconfig_file(sn_info_list) != 0: #20231102 David add
						sendlog("[RACKSN:{0};SN:{1}] remove_auto_reconfig_file FAIL !".format(sn_info_list["RACKSN"],sn_info_list["sn"]))
					...
					# if not os.path.isfile(prov_log_path): #20231024 David fix
					# 	if is_provision(sn_info_list) != 0:
					# 		clear_flag(sn_info_list)
					# 		return 1
					...
					if do_soc_fw_chk(sn_info_list) != 0:
						sendlog("[RACKSN:{0};SN:{1}] do_soc_fw_chk FAIL !".format(sn_info_list["RACKSN"],sn_info_list["sn"]))
						clear_flag(sn_info_list)
						return 1
			...
			if sn_info_list["uut_ip"] == 1 or sn_info_list["uut_ip"] == "":			
				...
				sendlog("[RACKSN:{0} SN:{1}] APP mode, do not need to do_record_cp_and_reconfig".format(sn_info_list["RACKSN"],sn_info_list["sn"]))
