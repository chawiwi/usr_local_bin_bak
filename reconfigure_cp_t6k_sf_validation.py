...
g_sf_config_folder = "/WIN/{}/sfconfig/test/".format(g_project.upper()) #QMF2
...
# root_path = os.path.abspath(__file__).split(__file__)[0][:-1]
...
def sendlog(message = "", color = 0, file = "/RACKLOG/t6k/RM_logs/monitor.log"):
	if color == 1 :
		ESC_color = ESC_GREEN
	elif color == 2 :
		ESC_color = ESC_RED
...
def create_auto_reconfig_file(MBSNdict):
	...
	log_path = "{0}/{1}".format(sn_log_folder, log_name)
	finish_flag_name = "{0}_test_finish.log".format(MBSNdict["mbsn"])
	finish_flag_path = "{0}/{1}".format(sn_log_folder, finish_flag_name)
	CP_IP=""

	if os.path.isfile(finish_flag_path):
		sendlog ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] {3} flag exist, not create auto reconfig!!".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"], finish_flag_path))
		return 0	
	
	if not os.path.isfile(auto_reconfig_service_server_loc):
		sendlog ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] File {3} not exist, please check it!!".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"], auto_reconfig_service_server_loc))
		return 1
	
	if not os.path.isfile(auto_reconfig_python_server_loc):
		sendlog ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] File {3} not exist, please check it!!".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"], auto_reconfig_python_server_loc))
		return 1
	
	#check auto reconfig exist
	print ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] check auto reconfig exist".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"]))
	...
	if auto_reconfig_service_soc_loc in result:
		sendlog ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] File {3} exist at soc.".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"], auto_reconfig_service_soc_loc))
		...
		if auto_reconfig_python_soc_loc in result:
			sendlog ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] File {3} exist at soc.".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"], auto_reconfig_python_soc_loc))
			if is_CP_Golden_mode(MBSNdict) == 2:
				try:
					if not MBSNdict["is_reconfig"]:
						sendlog ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Do reconfig Celestial Peak.".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"]))
						sendlog ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Need reconfig Celestial Peak.".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"]))
					...
					sendlog ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Pxssh failed: reconfig Celestial Peak.".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"]))
					sendlog(e, RED)
					return 1
				
			else:
				sendlog ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Auto reconfig PASS!!".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"]))
				return 0
			
	sendlog ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Create auto reconfig Celestial Peak.".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"]))	
	
	#Find CP SOC IP
	print ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Find CP SOC IP".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"]))
	...
	print ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Create {3} folder".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"], g_cmd_mkdir_service_loc_path))
	...
	sendlog(result, 0, log_path)	
	if result == "":
		sendlog ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Create service path fail!DBG:{3}".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"], result))
		return 1
	else:
		sendlog ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Create service path PASS!DBG:{3}".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"], result))
	...
	print ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] copy  SoCFPGATestSvc.service file to CP console".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"]))
	...
	time.sleep(15)
	if is_CP_Golden_mode(MBSNdict) == 2:
		sendlog ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Auto reconfig FAIL!!".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"]))
		return 1
	else:
		sendlog ("[mbsn:{0} cp_ip: {1} cp_mac: {2}] Auto reconfig PASS!!".format(MBSNdict["mbsn"], MBSNdict["cp_ip"], MBSNdict["cp_mac_addr"]))
		return 0
