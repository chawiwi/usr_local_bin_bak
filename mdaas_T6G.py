...
station_list = ['MDAASGENCONFIG','MDAASLINUX','MDAASWINDOWS', 'FST', 'NETTEST']
mdaas_station_list = ['MDAASGENCONFIG','MDAASLINUX','MDAASWINDOWS']
...
def check_start_time_flag(log_type, sf_sn):
	...
	if (log_type.upper() == "FAIL"):
		...
		if log_type.upper() == "PASS":
			if (get_station(sf_sn) == station_list[2]):
					os.system("rm -f {0}/{1}.txt".format(mdaas_dir, sf_sn))
					os.system("rm -f " + running_flag)
...
def get_racksn(sf_sn): #20231127 add web api use
    win_response = "{0}/{1}/response/config".format(win_dir, project)
    sf_conf_file = "{0}/{1}.txt".format(win_response, sf_sn)
    if os.path.exists(sf_conf_file):
...
def send_data_sf(message, log_type, sf_sn, mdaas_mode, start_time):
	...
	racksn = get_racksn(sf_sn) #20231127 add web api
	...
	elif api_mode == "Y": #20231127 add web api
	...
	for line in text:
		...
		for mac in line.split(","):
			...
			if len(uut_ip.split('.')) == 4:
				...
				cmd = "ping -c 5 {0} > /dev/null".format(uut_ip)
				if os.system(cmd) == 0:
						sendlog(log)
						ip = uut_ip 
...
def goto_next_station(sf_sn):
	...
	if check_in_mdaas(sf_sn, station_list[0]): #Check for MDAASGENCONFIG
		...
		next_station = station_list[1] # MDAAS_LINUX
		adjust_config_station(sf_sn, local_sf_file,sf_conf_file, current_station, next_station)
		return 0
	elif check_in_mdaas(sf_sn, station_list[1]): #Check for MDAAS_LINUX
		current_station = station_list[1]
		next_station = station_list[2] # MDAAS_WIN
		adjust_config_station(sf_sn, local_sf_file,sf_conf_file, current_station, next_station)
		return 0
	elif check_in_mdaas(sf_sn, station_list[2]): #Check for MDAAS_WIN
		info = get_txt_info(sf_conf_file)
		current_station = station_list[2]
		next_station = station_list[3] # FST
		if "GEN8.1" in info or "T6G" in project.upper() or "T6H" in project.upper(): #20231026 David modify: T6G/T6H have NETTEST
			next_station = station_list[4] # NETTEST
		adjust_config_station(sf_sn, local_sf_file,sf_conf_file, current_station, next_station)
		return 0
	else:
...
def get_rm_ip(sf_sn):
	...
	if os.path.exists(sf_conf_file):
		...
		if "T6G" in project.upper(): #20231026 David add: T6G/T6H have 2 RM
			...
			rackpn = getresult("cat {0} | grep -i 'RACKPN'".format(sf_conf_file))
			#RACKPN 1L15U9Z0060 is Supernova SKU.
			if rackpn != "1L15U9Z0060":
				if location >= "F27":
					RACK_MOUNT_MAC = getresult("cat {0} | grep -i 'RACK_MOUNT1_MAC1'".format(sf_conf_file))
				elif location <= "F20":
					RACK_MOUNT_MAC = getresult("cat {0} | grep -i 'RACK_MOUNT_MAC1'".format(sf_conf_file))
			...
			if status == "finished":
				...
				if status_code == 200 and mdaas_mode == "L10__genconfig": #L10__genconfig is golden config generating step
					...
					if (get_station(chs_sn) == "MDAASGENCONFIG"):
						goto_next_station(chs_sn) #Go to Linux
					
					mdaas_cmd = "curl -X POST -k https://{0}:5003/mdaas/workflow -H 'Content-Type: application/json' -d ".format(ip)
					mdaas_cmd = mdaas_cmd + "'{\"flow\": \"L10\", \"site\": \"QMF\"}'" # Original
					#mdaas_cmd = mdaas_cmd + "'{\"flow\": \"L10\", \"site\": \"QMF\",\"stage\": \"Beta\", \"validation\": \"true\"}'" # Beta
					logging.info(mdaas_cmd)
					ret = getresult(mdaas_cmd)
					time.sleep(1)
					sendlog("{0} Linux Starting".format(chs_sn))
					if api_mode == "Y": #20231127 add web api
						send2sf("{0} Linux Starting".format(chs_sn), "STARTING", chs_sn, "MDAASLINUX")
				...
				genconfig_cmd = genconfig_cmd + "'{\"genconfig_mode\": \"true\", \"site\": \"QMF\", \"flow\": \"L10\"}'" # Original 
				#genconfig_cmd = genconfig_cmd + "'{\"genconfig_mode\": \"true\", \"site\": \"QMF\", \"flow\": \"L10\",\"stage\": \"Beta\"}'" # Beta
...
if __name__ == "__main__":
	...
	if len(sys.argv) > 2: #20240131 David add for WebAPI switch
		if sys.argv[2] == "API":
			api_mode = "Y"
		else:
			api_mode = "N"
