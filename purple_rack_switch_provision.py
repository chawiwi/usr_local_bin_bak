...
current_8_point_1_lm = ("M1288874-001", "M1289494-001", "M1289493-001", "M1271465-001","M1271433-001")
...
async def run_y_cable_update(ip, log, model):
    ...
    try:
        async with asyncssh.connect(
            ip, username="admin", password="Temp123!", known_hosts=None,keepalive_interval=30,  keepalive_count_max=300
        ) as ssh_session:
            print(f"Connected to {ip} for y-cable update")

            # Determine the correct command based on the msfpn
            if model == "8.1hh":
                command = "sudo bash -c 'nohup python3 -u /host/FW_Update/mux_update_purple.py 8.1HH | tee -a /host/FW_Update/mux_update_purple.log'"
            elif model == "8.1lm":
                command = "sudo bash -c 'nohup python3 -u /host/FW_Update/mux_update_purple.py 8.1LM | tee -a /host/FW_Update/mux_update_purple.log'"
            else:
                print("Unknown model")
                return None
            ...
            process = await ssh_session.create_process(command, stderr=asyncssh.PIPE)
            output = ""

            # Stream the stdout in real-time as the command runs
            ...
            # Stream the stderr in real-time
            ...
            if process.exit_status == 0:
                ...
                print(f"Y-cable update failed on {ip} with exit status {process.exit_status}")
                return 1

    except asyncssh.Error as e:
        with open(log, "a") as file:
            file.write("STDERR: SSH connection failed: " + str(e))
        return None
def config_switch(sn_info, switch):
    ...
    try:
        ...
        for attempt in range(3):
            if send_mux_firmware_to_switch(ip, log) is not None:
                break  # Success, exit the loop
            else:
                failure_message = (
                    f"Attempt {attempt + 1} failed to transfer AEC firmware."
                )
                print(failure_message)
                with open(log, "a") as file:
                    file.write(f"{datetime.now()} - ERROR: {failure_message}\n")
        else:
            # If we exit the loop without breaking, raise the RuntimeError
            error_message = "Failed to transfer AEC firmware to /usr/share/sonic/firmware/ after 3 attempts"
            print(error_message)
            with open(log, "a") as file:
                file.write(f"{datetime.now()} - ERROR: {error_message}\n")
